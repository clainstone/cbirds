...
void update_birds(bird_t **birds, bird_t **close, int screen_width, int screen_height,
		  int num_birds);

void update_birds_index(bird_t **birds_copy_to_read, bird_t **birds_to_write, int screen_width,
			int screen_height, int start_index, int end_index, int birds_num);
static void update_direction(bird_t *bird, double next_direction);
static double calculate_rules_direction(bird_t *bird, bird_t **birds, int num_birds,
					double separation_weight, double alignment_weight,
					double cohesion_weight, double boundary_av_weight,
					int screen_width, int screen_heigth, int turn_radius);
static vector2d_t *calculate_boundary_av_direction(bird_t *bird, int screen_width,
...
static void close_birds(bird_t **close_birds_list, bird_t *target, bird_t **birds, int num_birds,
...
static double calculate_rules_direction(bird_t *target, bird_t **birds, int num_birds,
					double separation_weight, double alignment_weight,
					double cohesion_weight, double boundary_av_weight,
					int screen_width, int screen_heigth, int turn_radius)
{
	...
	vector2d_t *boundary_av_ptr =
		calculate_boundary_av_direction(target, screen_width, screen_heigth, turn_radius);
		  ...
		  int num_birds)
{
	for (int i = 0; i < num_birds; i++) {
		int counter = 0;
		close_birds(close, birds[i], birds, num_birds, PERCEPTION_RADIUS_SQUARED, &counter);
		...
		if (close != NULL && counter > 0) {
			double direction = calculate_rules_direction(
				...
				COHESION_WEIGHT, BOUNDARY_AV_WEIGHT, screen_width, screen_height,
				TURN_RADIUS);
	...
	for (int i = start_index; i < end_index; i++) {
		...
		if (counter > 0) {
			double direction = calculate_rules_direction(
				...
				ALIGNMENT_WEIGHT, COHESION_WEIGHT, BOUNDARY_AV_WEIGHT, screen_width,
